...
connected(Socket, Transport, Endpoint, Proto, RTSourceConnMgrPid, _Props) ->
  gen_server:cast(RTSourceConnMgrPid, {connected, Socket, Transport, Endpoint, Proto, _Props}).
...
maybe_rebalance_delayed(Pid) ->
  io:format("We do not rebalance yet, this will be fixed later ~n"),
  ok.
...
handle_cast({connected, Socket, Transport, Endpoint, Proto, _Props},
    State = #state{supervisor = S, remote = Remote}) ->

  case S:add_connection(Remote, Socket, Transport, Endpoint, Proto, _Props) of
    {ok, Pid} ->
